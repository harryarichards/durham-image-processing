...
def neighbours(x, y, img):
    ...
    :param img: the image we're dilating.
...
def dilate(img, out):
    """
    dilates the image provided.
    dilating involves setting a pixels value to the maximum of it and its neighbouring pixels values.
    neighbouring pixels are any pixel within 2 pixels of the pixel in question.
    also writes the dilated image to a file as well as returning the dilated image.

    :param img: the image we're dilating.
    :param out: the path we're writing the dilated image to.
    :return: the dilated image.
    """
    # Set h (height) equal to the number of rows in the image.
    # Set w (width) equal to the number of columns in the image.
    h, w = img.shape
    # Set the dilated image to a copy of the image that's been passed in.
    ...
    for x in range(0, w):
        ...
        for y in range(0, h):
            ...
            #Set the pixel in the dilated image with coordinates [x,y] equal to the maximum of the pixels in the neighbours array.
            dilated_img[x][y] = max(neighbours_array)
    # Write the dilated image file to the directory that was passed into the function.
    cv2.imwrite(out, dilated_img)
...
if __name__ == '__main__':
    ...
    # The dilate function is called on the image, and provided a pathway to write the image out to, - this is the second string input via the command line.
    dilate(img, sys.argv[2])